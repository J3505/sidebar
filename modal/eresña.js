...
function renderReviews() {
    ...
    reviewCard.innerHTML = `    
            ...
            <div class="p-6 bg-[#fff3d6]">
                ...
                <div class="flex justify-end mt-4">
                    <button class="bg-yellow-500 text-white px-4 py-2 rounded-lg mr-2 edit-btn-reseña" data-index="${index}">Editar</button>
                    <button class="bg-red-500 text-white px-4 py-2 rounded-lg delete-btn-reseña" data-index="${index}">Eliminar</button>
                </div>
            </div>
        `
  ...
  document.querySelectorAll('.edit-btn-reseña').forEach((button) => {
  ...
  document.querySelectorAll('.delete-btn-reseña').forEach((button) => {
    button.addEventListener('click', (e) => {
      const index = e.target.dataset.index
      deleteReview(index)
    })
