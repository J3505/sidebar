...
function renderOpciones() {
  ...
  opciones.forEach((item, index) => {
    ...
    optionCard.innerHTML = `
      ...
      <div class="flex mt-4">
        <button class="bg-yellow-500 text-white px-2 py-1 rounded-lg mr-2 edit-btn-opciones" data-index="${index}">Editar</button>
        <button class="bg-red-500 text-white px-2 py-1 rounded-lg delete-btn-opciones" data-index="${index}">Borrar</button>
      </div>
    `
  ...
  document.querySelectorAll('.edit-btn-opciones').forEach((button) => {
  ...
  document.querySelectorAll('.delete-btn-opciones').forEach((button) => {
    button.addEventListener('click', (e) => {
      const index = e.target.dataset.index
      deleteOption(index)
    })
